...
import { DATA_COUNTRY_BLOG, DATA_COUNTRY_BLOG_TITLE } from '@/graphql/country/queries'
...
async function OurBlog({ lang,slug }) {
  ...
  const dataCountryTitle = await fetchData(DATA_COUNTRY_BLOG_TITLE, {
    language: lang?.toUpperCase(),
    taxonomyValue: slug
  })

  const dataInfo = dataCountry?.data?.blogs?.nodes
  const dataInfoTitle = dataCountryTitle?.data?.countries?.translation

  const checkDataInfo = dataInfo?.filter((value) => value && value)


  if(!dataInfo?.length || checkDataInfo?.length === 0) {
    return null
  }
  return (
    <div className={`flex flex-col mt-[14.93vw] md:mt-[8.28vw] content`}>
      <span className='heading-1 md:mb-[3vw] mb-[6.4vw] md:pl-0 pl-[4.27vw]'>{dataInfoTitle?.ourTour?.titleBlogs}</span>
        ...
        <div className='flex flex-shrink-0 md:col-span-2 md:row-span-2 h-full md:pl-0 pl-[4.27vw]'>
          {dataInfo?.slice(0, 1).map((item, index) => (
            <div key={index} className='h-full'>
              ...
              <BlogItem2 data={item?.translation} lang={lang}/>
              </div>
              <div className='block max-md:hidden h-full'>
              <BlogItem2 data={item?.translation} lang={lang} className='bigger' />
              </div>
            </div>
          ))}
        </div>
        {dataInfo?.slice(1, 5)?.map((blog, index) => (
          <BlogItem2 data={blog?.translation} lang={lang} key={index} className='md:pl-0 pl-[4.27vw] flex flex-shrink-0 blogItem ' />
        ))}
      </div>
